...
namespace Web_Api_interface.Controllers
{
    ...
    public class LoggedInCustomerFacadeController : LoggedInFacadeControllerBase, ILoggedInCustomerFacadeController
    {        
        ...
        /// </summary>
        /// <returns></returns>
        [CustomAuthorize(Roles = "Customer, Administrator", UnathorisedRequestCusstomMessage = "asasasasasasasasasasasa")]
        ...
        public IHttpActionResult GetAllCustomers()
        {
            ...
            Action act = () => 
            {
                ...
                tsk = _loggedInCustomerFacade.PleaseGiveMeAll<Customer>(out string rabbitSubscriptionId);
                ...
                using (IBus bus = RabbitHutch.CreateBus("host=localhost"))
                {
                    while (customers == null)
                    {
                        ...
                        Thread.Sleep(100);
                        var subsciptionRezult = bus.SubscribeAsync<List<Customer>>(rabbitSubscriptionId, allCustomersLst => { return Task.Run(() => { customers = allCustomersLst; }); });    
                        count++;
                    }
                }
            ...
            if (customers == null) return ResponseMessage(Request.CreateErrorResponse(HttpStatusCode.InternalServerError, $"Sorry, but something wrong happened on the server, so your request can't be answered."));
            if (customers.Count == 0) return ResponseMessage(Request.CreateErrorResponse(HttpStatusCode.NoContent, $"There are no customers in the system."));

            //List<CustomerData> customersData = new List<CustomerData>();

            return Ok(CreateCustomerDataIE(customers));
        }
        private IEnumerable<CustomerData> CreateCustomerDataIE(IEnumerable<Customer> customers)
        {
        ...
        [HttpGet]
        [Route("PreloadAllCustomersIDs", Name = "CustomerPreloadAllCustomersIDs")]
        public async Task<IHttpActionResult> PreloadAllCustomersIDs()
        {
            List<long> customrsIDs = null;
            Func<Task<List<long>>> func = async() => 
            {
                return await _loggedInCustomerFacade.PreloadAllCustomerIDsAsync();
            };
            customrsIDs = await ProcessExceptionsAsync(func);

            return Ok(customrsIDs);            
        }         

        [CustomAuthorize(Roles = "Customer, Administrator", UnathorisedRequestCusstomMessage = "asasasasasasasasasasasa")]
        [HttpGet]
        [Route("TakeOneCustomer/{customerID}")]
        public async Task<IHttpActionResult> TakeOneCustomer([FromUri] int customerID)
        {
            return await Task.Run(async() => 
            {
                Task<string> tsk = null;
                tsk = _loggedInCustomerFacade.PleaseGiveMeOneAsync<Customer>(customerID);
                ...
                return Ok(CustomerToCustomerData(customer));
            }); 

        }


        private  CustomerData CustomerToCustomerData(Customer customer)
        {
